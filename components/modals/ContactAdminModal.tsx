...
const ContactAdminModal: React.FC<ContactAdminModalProps> = ({ isOpen, onClose, selectedPremium }) => {
    ...
    return (
            <div className="fixed inset-0 bg-slate-900/60 flex items-end sm:items-center justify-center z-[100] p-0 sm:p-6 touch-manipulation" onClick={resetAndClose}>
                <div className={`bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl ring-1 ring-slate-900/5 w-full sm:w-auto sm:max-w-xl overflow-hidden ${arClass} animate-slide-in-up ${bodyTextSize}`} role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()} dir={dir}>
                {/* Drag handle (mobile) */}
                <div className="sm:hidden w-full flex justify-center pt-2">
                    <div className="w-12 h-1.5 rounded-full bg-slate-300"></div>
                </div>
                {/* Header */}
                    <div className="px-5 py-3 sm:py-4 border-b border-slate-200 sticky top-0 bg-white z-10">
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2 text-gray-800 min-w-0">
                            <i className="fas fa-unlock text-teal-600"></i>
                            <h2 className="text-sm sm:text-base font-semibold tracking-tight truncate">{t.modalTitle}</h2>
                        </div>
                ...
                {/* Body */}
                    <div className="p-4 sm:p-5 space-y-4 sm:space-y-5 max-h-[72vh] sm:max-h-[75vh] overflow-y-auto leading-relaxed overscroll-contain">
                    ...
                    {step === 1 && (
                            ...
                            <p className="text-sm text-slate-700">{t.selectService}</p>
                                <div className="grid grid-cols-1 gap-3">
                                {SERVICE_OPTIONS.map(opt => (
                                    <button
                                        key={opt.id}
                                            className={`text-left px-4 py-3 rounded-xl border transition-colors ${service === opt.id ? 'border-indigo-500 bg-indigo-50 ring-1 ring-indigo-200' : 'border-slate-200 hover:border-slate-300' } flex items-center justify-between`}
                                        onClick={() => setService(opt.id)}
                                    >
                                            <div className="text-sm font-semibold">{lang === 'fr' ? opt.labelFr : opt.labelAr}</div>
